import os
from converters.ppt_to_pdf import convert as to_pdf
...
def _via_pdf(input_path: str, output_path: str, fmt: str, progress_cb=None):
    ...
    pdf_path = os.path.join(work_dir, "_tmp_ppt.pdf")

    def cb1(pct, stage=""):
        if progress_cb:
            progress_cb(int(pct * 0.5), stage)

    to_pdf(input_path, pdf_path, cb1)

    def cb2(pct, stage=""):
        if progress_cb:
            progress_cb(50 + int(pct * 0.5), stage)
